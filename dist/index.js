...
import { handleReadFileWithAutoChunking, handleListDirectoryWithAutoChunking } from './enhanced-handlers.js';
...
function getToolsByCategory(category, allToolNames) {
    const prefixes = {
    ...
    return allToolNames.filter(tool => categoryPrefixes.some(prefix => tool.startsWith(prefix) || tool === prefix));
}
// Expand disabled tools to include tools from categories
function expandDisabledTools(disabledTools, allToolNames) {
    ...
    if (content_search) {
        try {
            ...
            for (const r of ripResults) {
                ...
                if (!resultMap.has(normalizedPath))
...
async function handleEditMultipleBlocks(args) {
    ...
    try {
        ...
        for (let i = 0; i < sortedEdits.length; i++) {
            ...
            const { old_text, new_text, line_number, mode = 'replace', expected_replacements = 1, word_boundary = false, case_sensitive = true } = edit;
            let changesCount = 0;
            let editResult = {
                edit_index: i + 1,
                mode: mode,
...
async function handleEditBlocks(args) {
    ...
    try {
        ...
        for (let i = 0; i < edits.length; i++) {
            const edit = edits[i];
            const { old_text, new_text, expected_replacements = 1, word_boundary = false, case_sensitive = true } = edit;
            if (!old_text || new_text === undefined) {
                editResults.push({
                    edit_index: i + 1,
...
async function handleCopyFile(args) {
    const { source, destination, overwrite = false, preserve_timestamps = true, recursive = true, create_dirs = true } = args;
...
async function copyDirectoryRecursive(source, destination, overwrite, preserveTimestamps) {
    await fs.mkdir(destination, { recursive: true });
    const entries = await fs.readdir(source, { withFileTypes: true });
    for (const entry of entries) {
        const sourcePath = path.join(source, entry.name);
        const destPath = path.join(destination, entry.name);
        if (shouldExcludePath(sourcePath))
            continue;
        if (entry.isDirectory()) {
...
async function handleMoveFile(args) {
    const { source, destination, overwrite = false, create_dirs = true, backup_if_exists = false } = args;
    const sourcePath = safePath(source);
    const destPath = safePath(destination);
    try {
        const sourceStats = await fs.stat(sourcePath);
...
async function handleDeleteFile(args) {
    const { path: targetPath, recursive = false, force = false, backup_before_delete = false, confirm_delete = true } = args;
...
async function handleBatchFileOperations(args) {
    const { operations = [], stop_on_error = true, dry_run = false, create_backup = false } = args;
    const results = [];
    let successCount = 0;
    let errorCount = 0;
    let backupDir = null;
    ...
    try {
        for (let i = 0; i < operations.length; i++) {
            ...
            try {
                ...
                const sourcePath = safePath(source);
                let result = { operation: op, source: sourcePath };
                if (dry_run) {
...
async function handleCompressFiles(args) {
    const { paths = [], output_path, format = 'zip', compression_level = 6, exclude_patterns = [] } = args;
    // Node.js 내장 모듈로 간단한 압축 구현
    // 실제 구현에서는 archiver 등의 라이브러리 사용 권장
    const outputPath = safePath(output_path);
    const resolvedPaths = paths.map((p) => safePath(p));
    try {
        // 압축할 파일들 수집
...
function shouldExcludeFromCompression(filePath, excludePatterns) {
...
async function collectFilesForCompression(dirPath, archivePath, fileList, excludePatterns) {
    const entries = await fs.readdir(dirPath, { withFileTypes: true });
    for (const entry of entries) {
        const fullPath = path.join(dirPath, entry.name);
...
async function createTarArchive(files, outputPath, format, compressionLevel) {
...
async function handleExtractArchive(args) {
    const { archive_path, extract_to = '.', overwrite = false, create_dirs = true, preserve_permissions = true, extract_specific = [] } = args;
...
async function extractTarArchive(archivePath, extractPath, overwrite, specificFiles) {
    // tar 명령어로 압축 해제
...
async function handleSyncDirectories(args) {
    const { source_dir, target_dir, sync_mode = 'update', delete_extra = false, preserve_newer = true, dry_run = false, exclude_patterns = ['.git', 'node_modules', '.DS_Store'] } = args;
...
async function syncDirectoryRecursive(sourcePath, targetPath, relativePath, results, options) {
    try {
...
function shouldExcludeFromSync(filePath, excludePatterns) {
...
async function cleanupExtraFiles(sourcePath, targetPath, relativePath, results, options) {
    try {
        const currentTargetPath = path.join(targetPath, relativePath);
